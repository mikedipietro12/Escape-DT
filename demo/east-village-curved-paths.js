/**
 * East Village (Hastings-Sunrise) — curved paths map + Step E cards.
 * Alternate map style: demo/east-village-standard-path.js
 */
...
const PRESETS = {
  "backtrack-lanes": {
    label: "Backtrack demo (Nanaimo → Penticton → Kamloops)",
    ids: ["s52", "s54", "s56", "s57"],
  },
  "west-east": {
...
function assignLabelSides(points) {
  ...
  points.forEach((p, idx) => {
    let above = p.lane > 0 ? true : idx % 2 === 0;
    if (
      p.lane === 0 &&
      idx > 0 &&
      Math.abs(p.x - points[idx - 1].x) < 52 &&
      sides[idx - 1] === above
    ) {
      above = !above;
    }
...
/** Spine stops sharing a cross-street label (e.g. two Kamloops) share one map column. */
function snapSpineXByCrossStreetLabel(points) {
  const groups = new Map();
  points.forEach((p) => {
    if (!isOnHastingsSpine(p.stop)) return;
    const label = mapLabelCrossStreet(p.stop.crossStreet, p.stop);
    if (!label) return;
...
function layoutHorizontalMapPoints(route) {
  ...
  snapSpineXByCrossStreetLabel(points);
  assignRouteLanes(points);
...
function getSpineLineY(lane) {
  return HMAP.ySpine - lane * HMAP.laneStep;
}

/** Westward backtrack steps up; resuming east drops back to the Hastings spine. */
function assignRouteLanes(points) {
  let lane = 0;
  points[0].lane = 0;
  for (let i = 1; i < points.length; i++) {
    const prev = points[i - 1];
    if (points[i].x < prev.x - 2) {
      lane += 1;
    } else if (lane > 0 && points[i].x >= prev.x - 2) {
      lane = 0;
    }
    points[i].lane = lane;
  }
...
function applyRouteGeometry(point) {
  const lineY = getSpineLineY(point.lane);
  point.lineY = lineY;

  if (isOnHastingsSpine(point.stop)) {
    point.dotY = lineY - HMAP.dotAboveLine;
  } else {
    const offsetFromBaseSpine = getStopMapY(point.stop) - HMAP.ySpine;
    point.dotY = lineY + offsetFromBaseSpine - HMAP.dotAboveLine;
  }
  point.y = point.dotY;
}
...
function curvedLaneTransitionEast(x, yFrom, yTo) {
  const hook = HMAP.curveBulge;
  // Backtrack at the east end: bulge east, rise to the upper lane at x.
  return `C ${x + hook} ${yFrom}, ${x + hook} ${yTo}, ${x} ${yTo}`;
}

function curvedLaneTransitionWest(x, lineYUpper, spineY) {
  const hook = HMAP.curveBulge;
  // Mirror of east backtrack: bulge west, drop to the original Hastings line at x.
  return `C ${x - hook} ${lineYUpper}, ${x - hook} ${spineY}, ${x} ${spineY}`;
}

function isResumeEastLeg(a, b, prev) {
  if (!prev || a.lane === 0 || b.lane !== 0) return false;
  if (b.x < a.x - 2) return false;
  const sameCrossStreet =
    mapLabelCrossStreet(a.stop.crossStreet, a.stop) ===
    mapLabelCrossStreet(b.stop.crossStreet, b.stop);
  return prev.x > a.x - 2 || (sameCrossStreet && b.x >= a.x - 2);
}

/** Route path on stacked lanes; dots sit above the stroke (see applyRouteGeometry). */
function getStackedLegPathD(a, b, prev) {
  const x1 = a.x;
  const x2 = b.x;
  const lineY1 = a.lineY;
  const lineY2 = b.lineY;
  const dotY1 = a.dotY;
  const dotY2 = b.dotY;
  const spineY = HMAP.ySpine;
  const fromOffSpine = isStopOffSpine(a.stop);
  const toOffSpine = isStopOffSpine(b.stop);
  const laneBump = b.lane > a.lane;
  const resumeEastOnLane = isResumeEastLeg(a, b, prev);
  const parts = [];

  if (fromOffSpine) {
    parts.push(`M ${x1} ${dotY1}`, `L ${x1} ${lineY1}`);
  } else {
    parts.push(`M ${x1} ${lineY1}`);
  }

  if (laneBump) {
    parts.push(curvedLaneTransitionEast(x1, lineY1, lineY2));
  } else if (resumeEastOnLane) {
    parts.push(curvedLaneTransitionWest(x1, lineY1, spineY));
  }

  const travelY = resumeEastOnLane ? spineY : laneBump ? lineY2 : lineY1;
  if (Math.abs(x1 - x2) > 0.5) parts.push(`L ${x2} ${travelY}`);
  if (toOffSpine) parts.push(`L ${x2} ${dotY2}`);
...
function getRouteMapLayout(route) {
  ...
  const legs = routeLegs.map((leg, i) => {
    ...
    const prev = leg.fromStopIndex > 0 ? points[leg.fromStopIndex - 1] : null;
    return {
      ...leg,
      x1: a.x,
      y1: a.lineY,
      x2: b.x,
      y2: b.lineY,
      pathD: getStackedLegPathD(a, b, prev),
      legIndex: i,
...
function computeMapViewBox(points, legs) {
  ...
  const ys = [getSpineLineY(maxLane) - 36, HMAP.ySpine + 22];
  ...
  legs.forEach((leg) => {
    xs.push(leg.x1, leg.x2, leg.x1 + HMAP.curveBulge * 2, leg.x1 - HMAP.curveBulge * 2.2);
    ys.push(leg.y1, leg.y2, leg.y1 - HMAP.curveBulge * 1.6);
  });
...
function renderMapStopSvg(point) {
  return `
    <g class="map-stop-group map-stop-group--hidden" data-stop-index="${point.idx}">
      <circle class="map-stop" cx="${point.x}" cy="${point.dotY}" r="5" />
    </g>
  `;
}
...
function drawMap(route, options = {}) {
  ...
  svg.innerHTML = `${renderMapStaticSvg()}<g class="map-route-layer"></g><g class="map-overlay-layer"></g>`;
  ...
  const overlayLayer = svg.querySelector(".map-overlay-layer");
  ...
  const labelGroupsSeen = new Set();
  points.forEach((p) => {
    overlayLayer.insertAdjacentHTML("beforeend", renderMapStopSvg(p));
    if (p.isLabelAnchor && !labelGroupsSeen.has(p.labelGroup.key)) {
      labelGroupsSeen.add(p.labelGroup.key);
      overlayLayer.insertAdjacentHTML("beforeend", renderMapLabelSvg(p.labelGroup));
    }
  });
...
async function init() {
  ...
  if (presetSelect) presetSelect.value = "backtrack-lanes";
  runRouteReview(routeFromPreset("backtrack-lanes"));
}

/**
 * Commercial Drive — curved / stacked-lane route map mockup.
 * Vertical spine: backtracking south steps left; resuming north curves back to center.
 * Off-spine legs (Victoria, Venables, Frances) use rounded corners.
 */
...
let allStops = [];
let currentRoute = [];
const mapAnimationGeneration = { standard: 0, curved: 0 };
...
/** Southward backtrack steps left; resuming north drops back to the main spine. */
function assignRouteLanes(points) {
  let lane = 0;
  points[0].lane = 0;
  for (let i = 1; i < points.length; i++) {
    const prev = points[i - 1];
    if (points[i].y > prev.y + 2) {
      lane += 1;
    } else if (lane > 0 && points[i].y <= prev.y + 2) {
      lane = 0;
    }
    points[i].lane = lane;
  }
...
function isResumeNorthLeg(a, b) {
  if (a.lane === 0 || b.lane !== 0) return false;
  return b.y < a.y - 2;
}
...
/** Split turnaround legs so the U-turn traces before the straight leg (matches north resume). */
function getCurvedStackedLegSegments(a, b) {
  const lineX1 = a.lineX;
  const lineX2 = b.lineX;
  const y1 = a.layoutY;
  const y2 = b.layoutY;
  const spineX = MAP.xCenter;
  const laneBump = b.lane > a.lane;
  const resumeNorth = isResumeNorthLeg(a, b);
  ...
  if (laneBump) {
    return [
      {
        pathD: `M ${lineX1} ${y1} ${curvedLaneTransitionSouth(y1, lineX1, lineX2)}`,
  ...
  if (resumeNorth) {
    return [
      {
        pathD: `M ${lineX1} ${y1} ${curvedLaneTransitionNorth(y1, lineX1, spineX)}`,
        durationMs: MAP.turnCurveDurationMs,
      },
      {
        pathD: `M ${spineX} ${y1} L ${spineX} ${y2}`,
        durationMs: Math.max(MAP.legDurationMs - MAP.turnCurveDurationMs, 900),
      },
    ];
  }
...
function getCurvedStackedLegPathD(a, b) {
  ...
  if (laneBump) {
    parts.push(curvedLaneTransitionSouth(y1, lineX1, lineX2));
  } else if (resumeNorth) {
    parts.push(curvedLaneTransitionNorth(y1, lineX1, spineX));
  }

  const travelX = resumeNorth ? spineX : laneBump ? lineX2 : lineX1;
    ...
    if (leg.legKind === "stationToStop") {
      ...
      if (curved) {
        if (isStopOffSpine(p.stop)) {
          ...
          pathD = `M ${p.lineX} ${p.layoutY} L ${MAP.xCenter} ${station.y}`;
        }
      } else {
...
function replayMaps() {
  ...
  drawMap(document.getElementById("route-map-standard"), route, {
    curved: false,
    mapKey: "standard",
  });
  drawMap(document.getElementById("route-map-curved"), route, {
    curved: true,
    mapKey: "curved",
  });
}
